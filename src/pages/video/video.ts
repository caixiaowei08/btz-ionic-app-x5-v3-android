...
export class VideoPage {
    ...
    this_.storage.get("currentPlayVideoItemId").then((data) => {
      ...
      for (let i = 0; i < this_.videos.length; i++) {
        for (let j = 0; j < this_.videos[i].list.length; j++) {
          ...
          cordova.plugins.CordovaFileTransfer.checkVideoItemState(videoItem.id, this_.file.dataDirectory, (result) => {
      ...
      cordova.plugins.CordovaFileTransfer.checkFileIsExist(this_.currentVideoItem.id, this_.file.dataDirectory, (result) => {
  ...
  download(i: number, j: number) {
    ...
    cordova.plugins.CordovaFileTransfer.download(videoDownload.id, videoDownload.videoUrl, this_.file.dataDirectory, (result) => {
      this_.presentToast("已添加到下载队列！");
    }, (error) => {
      if (error == "exist") {
        cordova.plugins.CordovaFileTransfer.stop(videoDownload.id, (result) => {
  ...
  deleteVideoFile(i: number, j: number) {
    let this_ = this;
    let videoDownload = this_.videos[i].list[j];
    cordova.plugins.CordovaFileTransfer.delete(videoDownload.id, videoDownload.videoUrl, this_.file.dataDirectory, (result) => {
