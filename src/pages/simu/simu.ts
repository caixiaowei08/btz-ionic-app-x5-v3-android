...
export class SimuPage {
  ...
  constructor(public alertCtrl: AlertController, public modalCtrl: ModalController, public httpstorage: HttpStorage, public storage: Storage, public navCtrl: NavController, public navParams: NavParams) {
    ...
    if (this.tt.length == 0) {
      ...
      this.httpstorage.getStorage("s" + this.subject.id + "s", (data) => {
        if (data != null) {
          ...
          //模拟考场
          this.httpstorage.getStorage("s" + this.subject.id + "r", (data) => {//所有的组卷记录
            if (data == null) {
              data = [];
            }
